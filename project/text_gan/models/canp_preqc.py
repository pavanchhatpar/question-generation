...
from .canp_preqc_encoder import CANP_PreQC_Encoder
...
class CANP_PreQC:
    ...
    @tf.function
    def forward_step(self, cis, cit, ans, ner, pos, qpre, qit, qis,
                     enc_hidden, epoch_no, batch_no, training):
        hd, hD, mask, cemb = self.encoder(
            cis, ans, ner, pos, qpre, enc_hidden, training)
        state = {
            "encoder_output": hd,
    ...
    def debug(
            self, cis, qit, ans, qpre, attn_weights,
        ...
        qtpre = self.vocab.inverse_transform(qpre.numpy(), "target")
        for i in range(cis.shape[0]):
            ...
            self.logger.debug(f"Target prefix:\n{qtpre[i]}")
    ...
    def train_step(self, X, y, epoch_no, batch_no):
        ...
        with tf.GradientTape() as tape:
            ...
            output_dict = self.forward_step(
                cis, cit, ans, ner, pos, qpre, qit, qis,
                enc_hidden, epoch_no, batch_no, training)
