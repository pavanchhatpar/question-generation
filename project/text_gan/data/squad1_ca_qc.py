...
class SQuAD_CA_QC:
    ...
    def parse_ex(self, example_proto):
        ...
        return ((cis, cit, ans, ner, pos), (qit, qis))
    ...
    def _prepare(self):
        ...
        def gen():
            for cis, cit, ner, pos, qit, qis, ans in zip(
                    train_cis, train_cit, train_ner, train_pos,
                    train_qit, train_qis, training_ans):
                yield (cis, cit, ner, pos, qit, qis, ans)

        train_dataset = tf.data.Dataset.from_generator(
            gen,
            (
                tf.int32, tf.int32, tf.uint8, tf.uint8,
                tf.int32, tf.int32, tf.uint8),
            (
    ...
    def make_example(self, cis, cit, ner, pos, qit, qis, ans):
        serialized = tf.py_function(
            self.serialize,
            [cis, cit, ner, pos, qit, qis, ans],
            tf.string
        )
    ...
    def serialize(self, cis, cit, ner, pos, qit, qis, ans):
